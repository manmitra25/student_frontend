...
export default function AssessmentRunner({
 ...
 const isComplete = answers.length === total && answers.every(a => a !== undefined);
 ...
 const handleSubmit = async () => {
  ...
  try {
   const type = title.toLowerCase() as "stress" | "depression" | "anxiety" | "burnout";
   await saveTestResult(type, { score });
   setSubmitted(true);
 ...
 return (
  ...
  <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
   ...
   <div className="text-xs text-muted-foreground text-center">{isComplete ? 'Completed' : `Question ${index + 1} of ${total}`}</div>
   ...
   {!isComplete ? (
     ...
     <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
       ...
       {options.map((opt) => (
        ...
        <Button key={opt.label} variant="outline" className="w-full justify-start" onClick={() => onSelect(opt.value)}>
      ...
      <div className="flex justify-between">
       ...
       <Button variant="ghost" size="sm" onClick={goToPreviousQuestion} disabled={index === 0}>Back</Button>

       <div className="text-xs text-muted-foreground">Your selections are private</div>
    ...
    <Card className="p-6 border-0 shadow-sm">
     ...
     <div className="space-y-3 text-center">
      ...
      <p className="text-sm text-muted-foreground max-w-md mx-auto">{result?.message}</p>
